...
class CardDeck:
    ...
    def get_totalvalue(self,cards):
        total=0
        for card in cards:
            total+=self.get_value(card)
        return total
...
def main():
    while True:
        ...
        if game_on=='y' or game_on=='Y':
            ...
            t_user=c.get_totalvalue(c.user_cards)
            t_computer=c.get_totalvalue(c.computer_cards)
            c.display_full()
            if t_user<=21 and t_user>t_computer:
